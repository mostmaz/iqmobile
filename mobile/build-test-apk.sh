...
# <application>, and the merger refuses two different labels. Without this
# the whole build dies at :app:processReleaseMainManifest.
m = m.replace('<application android:name=".MainApplication"',
              '<application tools:replace="android:label" '
              'android:usesCleartextTraffic="true" android:name=".MainApplication"', 1)
